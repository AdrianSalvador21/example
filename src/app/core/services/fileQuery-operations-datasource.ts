...
import {FileQueryOperation} from '../../shared/models';
import {AuthorizationService} from './authorization.service';
import {FileQueryService} from './fileQuery.service';
import * as moment from 'moment';
import {Moment} from 'moment';
import {AppConfig} from '../../configs/app.config';

export class FileQueryDataSource implements DataSource<FileQueryOperation> {

  private operationSubject = new BehaviorSubject<FileQueryOperation[]>([]);
  ...
  connect(collectionViewer: CollectionViewer): Observable<FileQueryOperation[]> {
    return this.operationSubject.asObservable();
  }

  getOperations(): FileQueryOperation[] {
  ...
  loadOperations(operationGroup, currentUser, creatorUser: string, clientID: string, dateFrom: Moment, dateTo: Moment, status: string, folio: string, cantSignatures: string, pageNumber = 1, pageSize = 10) {
    ...
    this.authorizationService.getOperations(operationGroup, currentUser, creatorUser, clientID, dateFrom, dateTo, status, folio, cantSignatures, pageNumber, pageSize).pipe(
      ...
      .subscribe(data => {
        ...
        this.operLength.next(parseInt(this.authorizationService.totalOperations, 2));
      });
  }
}
